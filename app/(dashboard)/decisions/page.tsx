...
export default async function DecisionsPage({
  ...
  if (!client) {
    return (
      <div className="p-8">
        <h1 className="text-2xl font-extrabold text-slate-900 mb-2">Centre de decisions</h1>
  ...
  return (
    <div className="p-8 w-full space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900">Centre de decisions</h1>
      ...
      <Carte>
        <form className="flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-[200px]">
            <label htmlFor="q" className="block text-xs font-semibold text-slate-500 mb-1">Rechercher</label>
            <input
          ...
          <div>
            <label htmlFor="categorie" className="block text-xs font-semibold text-slate-500 mb-1">Categorie</label>
            <select
              id="categorie" name="categorie" defaultValue={categorie ?? 'TOUTES'}
              className="px-3 py-2 rounded-lg border border-slate-200 bg-[#FAF9F6] text-sm
                         focus:outline-none focus:border-indigo-400 focus:bg-white"
            >
          ...
          <button
            type="submit"
            className="bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-semibold px-4 py-2.5 rounded-lg"
          >
            Filtrer
          </button>
        </form>
      ...
      >
        {decisions.length > 0 ? (
          <div className="space-y-2 mb-4">
            {decisions.map((d) => (
              <div key={d.id} className="group rounded-xl border border-slate-200 px-4 py-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {d.context && (
                      <p className="text-sm text-slate-500 mt-1 leading-relaxed">{d.context}</p>
                    )}
                  </div>
