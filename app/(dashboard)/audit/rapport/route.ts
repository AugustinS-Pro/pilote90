...
/**
 * Rapport comptable en PDF, genere entierement cote serveur.
 *
...
export async function GET() {
  ...
  const indicateurs = calculerIndicateurs(transactions, cycle?.caTargetMonthly ?? 0, aujourdhui)
  ...
  pdf.setTitle(`Pilote90 — rapport comptable — ${client.companyName}`)
  ...
  enTete(doc, client.companyName, periode)
  ...
  pieds(doc, client.companyName, genereLe)

  const octets = await pdf.save()
  const horodatage = aujourdhui.toISOString().slice(0, 10)

  return new Response(Buffer.from(octets), {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename="pilote90-rapport-${horodatage}.pdf"`,
